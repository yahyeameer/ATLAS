...
- Review each candidate against every §15 gate; a single failed gate means request changes.
- Check FTMO 2-Step fit: 5% daily loss from day-open balance, 10% static max loss, internal buffers per PRD §19.
- Propose risk changes as findings only; the operator applies them with a signed change.
